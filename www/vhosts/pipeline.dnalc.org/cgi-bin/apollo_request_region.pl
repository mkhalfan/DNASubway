...
my $javacmd = "$apollo -H -w $working_dir/$filename.xml -o $xmltype -l $filename -i chadoDB";
...
if (!$error_flag) { 
    print  p(
			"Click "
			. a({href=>"$hostname/$web_path/$filename.jnlp", -style=> 'text-decoration: underline'},"here")
			." to start Apollo."
		);

	print p(
			"When you have finished editing, save the changes as GAME .xml file and upload it "
			.  a({href=>"$hostname/cgi-bin/upload_game.pl", -style=> 'text-decoration: underline'}, "here.")
		);
}
