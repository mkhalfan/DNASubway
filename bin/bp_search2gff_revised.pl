...
# Modified:	   Zhenyuan Lu <luj-at-cshl-dot-edu>
...
sub shatter_hit {
	my $hit = shift; #this is the hit object generated by bioperl parser
	my $ref = ref($hit); #this allows me to always generate the correct hit subtype (Bio::Search::Hit::GenericHit or Bio::Search::Hit::Blast)
	my @new_hits;
	foreach my $hsp ($hit->hsps) { #iterate through all HSPs
		my $new_hit = new $ref(
			'-name'         => $hit->name,
		...
		$new_hit->{is_shattered} = 1; #just a tag I force into the object structure
		$new_hit->add_hsp($hsp);
